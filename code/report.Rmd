...
imp_single <- mice(data, m = 1)
```

```{r dens-single, echo = FALSE}
# depression levels
...
```{r freqs-single, echo = FALSE}
ggmice(imp_single, aes(dep2, fill = .imp)) +
  geom_bar()

ggmice(imp_single, aes(dep3, fill = .imp)) +
  geom_bar()

ggmice(imp_single, aes(dep5, fill = .imp)) +
  geom_bar()

ggmice(imp_single, aes(dep6, fill = .imp)) +
  geom_bar()

ggmice(imp_single, aes(dep8, fill = .imp)) +
  geom_bar()

ggmice(imp_single, aes(dep9, fill = .imp)) +
  geom_bar()
```

```{r model-single, echo = FALSE}
...
```{r imp_multiple, echo = FALSE}
imp_multiple <- mice(data, m = 5)
```
...
```{r model-multiple, echo = FALSE}
# Combine depression levels post-hoc (might not be the best method).
imp_multiple <- with(imp_multiple, combine_dep)
