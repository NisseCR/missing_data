...
The 4th and last imputation model generated 5 imputed datasets. From Figure \@ref(tab:ratio-outcome-mi) it was observed that the ratio of "yes" to "no" cases in the imputed outcome variable `drink_regularly` followed the original ratio closely. The resulting data in each of the 5 datasets therefore showed little changes in the distribution of the outcome variable.  
...
ggmice(imp_multiple, aes(drink_regularly)) +
  ...
  facet_wrap(~.imp)
```
Figure \@ref(fig:dens-multiple) shows the density plots for the continuous variables in the dataset. Similar to the stochastic regression analysis, the imputed values for the continuous

```{r dens-multiple, fig.cap="Density plots for multiple imputation", echo = FALSE, warning = FALSE}
densityplot(imp_multiple)
```
When first observing the convergence of the imputation models, it was ascertained that `weight` and `bmi` did not converge. Specifically, the mean values of `weight` throughout the 5 iterations of imputing did not intermingle with one another, similar observations were made for `bmi`. In this case, `bmi` is a derived variable based on `weight` 
