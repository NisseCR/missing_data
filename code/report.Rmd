...
TODO: determine derived variable imputation (in our case the sum of all the depression levels).
...
imp_multiple_dev <- as.mids(comp_multiple)
```
...
```{r method-single, echo = FALSE}
imp_single$method
```

```{r dens-single, echo = FALSE, warning = FALSE}
# depression levels
densityplot(imp_single)

# drink_regularly
ggmice(imp_single, aes(drink_regularly, fill = .imp)) +
  geom_histogram(stat = 'count')
```
...
# drink_regularly
ggmice(imp_multiple, aes(drink_regularly, fill = .imp)) +
  geom_histogram(stat = 'count')
```
