...
dep_comparison <- function(org, imp) {
  dep_means_org <- dep_means(org, 'original')
  dep_means_imp <- dep_means(imp, 'imputed')
...
ratio_change <- perc(1 - ratio_lw / ratio_main)

# depression means
deps_lw <- dep_comparison(main_data, data_lw)
```

The data before applying list-wise deletion had `r main_n` cases. After performing the deletion-based treatment, the resulting dataset had `r lw_n` cases left. The ratio of "yes" to "no" cases in the outcome variable `drink_regularly` changed from `r ratio_main` to `r ratio_lw`, hence decreasing by `r decrease`%. The change in the mean of the depression levels can be seen in Figure @\ref(fig:dep-means-lw).

```{r dep-means-lw, echo = FALSE}
show_table(deps_lw, "Depression means for list-wise deletion")
```
